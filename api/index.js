...
const MEAL_TIME_SETTINGS_TABLE = "meal_time_settings";
...
function isMissingBirthdayColumn(error) {
  const message = String(error?.message || error?.details || "");
  return (message.includes("birthday") || message.includes("blood_group")) && message.toLowerCase().includes("column");
}

function missingBirthdayColumnError() {
  return new Error("Missing database column: public.members.birthday / blood_group. Run birthday-and-blood-group-extension.sql in Supabase, then save the member again.");
}

// ─── Member "last updated" touch (used to order the live dashboard feed) ──────
async function touchMemberUpdatedAt(db, email) {
  ...
  try {
    ...
    console.error("Failed to touch members.updated_at:", e);
  }
}
  ...
  try {
    ...
    if (action === "getData") {
      ...
      const [
        ...
        db.from(MEAL_TIME_SETTINGS_TABLE).select("*").eq("id", 1).maybeSingle(),
      ...
      const lunchCutoffHour  = Number.isFinite(Number(mealTimeSettingsRes.data?.lunch_cutoff_hour))
        ? Number(mealTimeSettingsRes.data.lunch_cutoff_hour) : 11;
      const dinnerCutoffHour = Number.isFinite(Number(mealTimeSettingsRes.data?.dinner_cutoff_hour))
        ? Number(mealTimeSettingsRes.data.dinner_cutoff_hour) : 18;
      ...
      // ── Member "last updated" map (drives the live roster ordering) ───────
      const memberUpdatedAtByEmail = {};
      allMemberDir.forEach(m => {
        memberUpdatedAtByEmail[String(m.email || "").toLowerCase().trim()] = m.updated_at || null;
      });
      // Diagnostic: Postgres/Supabase silently omits a column that doesn't
      // exist yet (select("*") won't error) — if every row is missing the
      // key entirely, the birthday-and-blood-group-extension.sql migration
      // hasn't been run, and the roster below will look "unsorted" because
      // every member falls back to the same rank. Surface this to the admin
      // instead of failing silently.
      const updatedAtColumnPresent = allMemberDir.some(m => Object.prototype.hasOwnProperty.call(m, "updated_at"));

      // Most-recently-updated member first. Falls back to 0 (epoch) for
      // members that predate the updated_at column, so they simply sort last.
      const activeRoster = [...todayMeals]
        .sort((a, b) => {
          const ta = Date.parse(memberUpdatedAtByEmail[String(a.member_email || "").toLowerCase().trim()] || 0) || 0;
          const tb = Date.parse(memberUpdatedAtByEmail[String(b.member_email || "").toLowerCase().trim()] || 0) || 0;
          return tb - ta;
        })
        .map((r, idx) => ({
          name: r.member_name,
          ...
          rank: idx + 1,
          updatedAt: memberUpdatedAtByEmail[String(r.member_email || "").toLowerCase().trim()] || null,
        }));

      // ── Birthday & pre-birthday countdown info ─────────────────────────────
      const serverTodayForBday    = todayStr();
      const serverTomorrowForBday = addDays(serverTodayForBday, 1);
      const todayMD    = monthDay(serverTodayForBday);
      const tomorrowMD = monthDay(serverTomorrowForBday);
      const todayCelebrants = allMemberDir
        .filter(m => m.birthday && monthDay(m.birthday) === todayMD)
        .map(m => ({ name: m.name, email: String(m.email || "").toLowerCase().trim() }));
      const tomorrowCelebrants = allMemberDir
        .filter(m => m.birthday && monthDay(m.birthday) === tomorrowMD)
        .map(m => ({ name: m.name, email: String(m.email || "").toLowerCase().trim() }));
      ...
      const allMembersRaw = await Promise.all(allMemberDir.map(async m => {
        ...
        if (activeMatch) {
          return {
            ...
            monthBazar: activeMatch.monthBazar,
            monthAdj: activeMatch.monthAdj,
            monthMealCost: activeMatch.monthMealCost,
            monthMealCount: activeMatch.monthMealCount,
      ...
      return sendJson(res, {
        ...
        activeRoster,
        rosterSortWarning: updatedAtColumnPresent
          ? null
          : "members.updated_at column not found — run birthday-and-blood-group-extension.sql in Supabase, then reload. Until then the live update list won't be sorted by recency.",
        birthday: {
          todayCelebrants,
          tomorrowCelebrants,
          // Start of "tomorrow" in Asia/Dhaka (fixed UTC+6, no DST) — this is
          // exactly the moment the upcoming birthday(s) begin, and therefore
          // the countdown target for the pre-birthday banner.
          countdownTargetIso: `${serverTomorrowForBday}T00:00:00+06:00`,
        },
        userTodayMeal,
        next7Preferences,
        mealDeadlineExtended,
        mealTimeSettings: { lunchCutoffHour, dinnerCutoffHour },
    ...
    if (action === "submitMeal") {
      ...
      if (error) throw error;
      await touchMemberUpdatedAt(db, email);
    ...
    if (action === "submitMealPreferences") {
      ...
      if (error) throw error;
      await touchMemberUpdatedAt(db, normalizedEmail);
    ...
    // ════════════════════════════════════════════════════════════════════════
    // ADMIN: SET MEAL SUBMISSION CUTOFF HOURS (lunch/dinner lock times)
    // ════════════════════════════════════════════════════════════════════════
    if (action === "adminSetMealTimes") {
      const lunchCutoffHour  = parseInt(req.body.lunchCutoffHour, 10);
      const dinnerCutoffHour = parseInt(req.body.dinnerCutoffHour, 10);
      if (!Number.isInteger(lunchCutoffHour)  || lunchCutoffHour  < 0 || lunchCutoffHour  > 23 ||
          !Number.isInteger(dinnerCutoffHour) || dinnerCutoffHour < 0 || dinnerCutoffHour > 23) {
        return sendJson(res, { success: false, error: "Cutoff hours must be whole numbers between 0 and 23." }, 400);
      }
      if (dinnerCutoffHour <= lunchCutoffHour) {
        return sendJson(res, { success: false, error: "Dinner cutoff must be later than lunch cutoff." }, 400);
      }
      const { error } = await db.from(MEAL_TIME_SETTINGS_TABLE).upsert(
        { id: 1, lunch_cutoff_hour: lunchCutoffHour, dinner_cutoff_hour: dinnerCutoffHour, updated_at: new Date().toISOString() },
        { onConflict: "id" }
      );
      if (error) throw error;
      return sendJson(res, { success: true, lunchCutoffHour, dinnerCutoffHour });
    }
    ...
    if (action === "adminOverrideMeal") {
      ...
      if (error) throw error;
      await touchMemberUpdatedAt(db, targetEmail);
      return sendJson(res, { success: true });
    }
    ...
    if (action === "adminSaveMember") {
      const { originalEmail, name, email, password, mobileNumber, authority, birthday, bloodGroup } = req.body;
      ...
      const row = {
        ...
        birthday:         birthday ? String(birthday).slice(0, 10) : null,
        blood_group:      String(bloodGroup || "").trim(),
        updated_at:       new Date().toISOString(),
      };
      ...
      if (isMissingBirthdayColumn(error)) throw missingBirthdayColumnError();
      if (error) throw error;
      return sendJson(res, { success: true });
    }
